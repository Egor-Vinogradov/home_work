Задание 1: Создан класс DataContainer у которого есть один дженерик.
Задание 2: Добавлен метод int add(T item).
Задание 3: Добавлено ДВА!!!!! конструктора
Задание 4: Добавлен метод int add(T item)
Задание 5: Добавлен метод T get(int index)
Задание 6: Добавлен метод T[] getItems()
Задание 7: Добавлен boolean delete(int index)
Задание 8: Добавлен boolean delete(T item)
Задание 9: Добавлен public void sort(Comparator<? super T> comparator)
Задание 10: Переопределен метод toString()
Задание 11:
Задание 12: Добавлен СТАТИЧЕСКИЙ метод void sort(DataContainer<?> container, Comparator<Object> comparator)
Задание 13: Реализован в DataContainer интерфейс Iterable